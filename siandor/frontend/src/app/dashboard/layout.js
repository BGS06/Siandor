...
export default function DashboardLayout({ children }) {
  ...
  const openModal = (jenis = "") => { setJenisSurat(jenis); setIsModalOpen(true); };
  const closeModal = () => { setIsModalOpen(false); setJenisSurat(""); };

  // Penentuan Judul Dinamis
  ...
  // LOGIKA TAMPILKAN TOMBOL TAMBAH SURAT
      ...
      <div className="flex min-h-screen bg-latar overflow-x-hidden">
        ...
        <aside className={`w-[260px] min-h-screen bg-hijau-tua flex flex-col fixed top-0 left-0 z-50 overflow-y-auto transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 shadow-2xl lg:shadow-none`}>
          <div className="pt-7 pb-6 px-6 border-b border-white/5 shrink-0 flex justify-between items-center">
            ...
            {/* BRANDING DENGAN LOGO TANPA BINGKAI/BACKGROUND PUTIH */}
            <div className="flex items-center gap-4">
              
              {/* LOGO KABUPATEN JOMBANG - STANDALONE ON GREEN */}
              <img 
                src="/logo-kabupaten.png" 
                alt="Logo Kabupaten Jombang" 
                className="w-11 h-11 object-contain rounded-md shadow-md shrink-0"
              />
          ...
          <nav className="flex-1 py-4 flex flex-col gap-1">
            ...
            <div className="px-6 pt-5 pb-1 text-xs text-white/40 tracking-widest uppercase font-bold">Surat Masuk & Keluar</div>
          ...
          <div className="mt-auto p-5 border-t border-white/5 shrink-0">
            <div className="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center gap-3 hover:bg-white/10 transition-colors cursor-pointer">
              <div className={`w-10 h-10 rounded-full ${activeProfile.color} flex items-center justify-center font-bold text-sm text-white shrink-0`}>
                {activeProfile.inisial}
              </div>
            ...
            <div className="flex items-center gap-2 lg:gap-4">
              ...
              <div className="relative">
                <div onClick={() => setIsNotifOpen(!isNotifOpen)} className={`w-10 h-10 rounded-xl border flex items-center justify-center cursor-pointer transition relative ${isNotifOpen ? 'bg-abu-muda border-hijau text-hijau' : 'bg-white border-border text-abu hover:bg-abu-muda'}`}>
                  ...
                  {unreadCount > 0 && <div className="absolute top-2.5 right-2.5 w-2 h-2 bg-merah rounded-full border-[1.5px] border-white animate-pulse"></div>}
                </div>
                {isNotifOpen && <div className="fixed inset-0 z-40" onClick={() => setIsNotifOpen(false)}></div>}
                
                {isNotifOpen && (
                  <div className="absolute right-0 mt-3 w-[300px] sm:w-[360px] bg-white border border-border rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
                    ...
                    <div className="max-h-[340px] overflow-y-auto">
                      {notifications.map((notif) => (
                        <div key={notif.id} onClick={() => handleNotifClick(notif)} className={`px-5 py-3.5 border-b border-latar flex gap-3 hover:bg-latar transition cursor-pointer ${notif.read ? 'opacity-70' : 'bg-white'}`}>
                          <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${notif.type === 'masuk' ? 'bg-emas-pale text-emas' : notif.type === 'warning' ? 'bg-[#FDECEA] text-merah' : 'bg-biru-muda text-biru'}`}>
                            {notif.type === 'masuk' && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg>}
                            {notif.type === 'warning' && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-start justify-between gap-2 mb-0.5">
                              <div className={`text-sm ${notif.read ? 'font-medium text-hitam' : 'font-bold text-hitam'}`}>{notif.title}</div>
                              {!notif.read && <div className="w-2 h-2 bg-hijau rounded-full mt-1.5 shrink-0"></div>}
                            </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm transition-opacity">
            <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-5 lg:p-6 border-b border-border flex items-center justify-between shrink-0 bg-white rounded-t-2xl">
                <div>
                  <h2 className="text-lg font-bold text-hitam">Tambah Surat Baru</h2>
                  <p className="text-sm text-abu mt-0.5">Isi form di bawah untuk menambahkan surat ke arsip.</p>
                </div>
              ...
              <div className="p-5 lg:p-6 space-y-5 overflow-y-auto flex-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value="AG-049/2026" disabled className="w-full p-3 border-border border rounded-xl text-sm bg-latar text-abu font-bold cursor-not-allowed" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-hitam mb-1.5 block">Jenis Surat <span className="text-merah">*</span></label>
                    <select className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white cursor-pointer transition">
                      <option value="">-- Pilih Jenis --</option>
                      <option>1. SURAT KETERANGAN KELAHIRAN</option>
                      <option>2. SURAT KETERANGAN KEMATIAN</option>
                      <option>3. SURAT KETERANGAN USAHA</option>
                      <option>4. SURAT KETERANGAN DOMISILI</option>
                      <option>5. SURAT KETERANGAN BELUM PERNAH NIKAH</option>
                      <option>6. SURAT KETERANGAN SUDAH MENIKAH</option>
                      <option>7. SURAT KETERANGAN KEPEMILIKAN KENDARAAN</option>
                      <option>8. SURAT KETERANGAN CATATAN KEPOLISIAN</option>
                      <option>9. SURAT KETERANGAN PENDUDUK</option>
                      <option>10. SURAT KETERANGAN PENGANTAR BBM</option>
                      <option>11. SURAT KETERANGAN PENGHASILAN</option>
                      <option>12. SURAT KETERANGAN TIDAK MAMPU</option>
                      <option>13. SURAT KETERANGAN KEHILANGAN</option>
                      <option>14. SURAT KETERANGAN UMUM / YANMA</option>
                      <option>15. SURAT KETERANGAN BEPERGIAN</option>
                      <option>16. SURAT PERNYATAAN DAN KUASA</option>
                      <option>17. SURAT DINAS KELUAR</option>
                      <option>18. SURAT DINAS DATANG</option>
                      <option>19. YAMKESMASKIN</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white transition" placeholder="Contoh: Budi / Kecamatan Paron" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-hitam mb-1.5 block">NIK</label>
                    <input type="text" className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white transition" placeholder="16 digit NIK (Bila ada)" />
                  </div>
                </div>

                <div>
                  <label className="text-sm font-semibold text-hitam mb-1.5 block">Perihal <span className="text-merah">*</span></label>
                  <input type="text" className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white transition" placeholder="Isi ringkas tujuan surat" />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-hitam mb-1.5 block">Nomor Surat Asli</label>
                    <input type="text" className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white transition" placeholder="Sesuai dokumen fisik" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-hitam mb-1.5 block">Tanggal Terima / Kirim <span className="text-merah">*</span></label>
                    <input type="date" className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white transition" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="border border-dashed border-border rounded-xl p-8 text-center hover:bg-hijau-pale hover:border-hijau-muda transition cursor-pointer group">
                    <svg className="mx-auto mb-3 text-abu group-hover:text-hijau transition" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select className="w-full p-3 border-border border rounded-xl text-sm outline-none focus:border-hijau bg-white cursor-pointer transition">
                      <option>Kepala Desa</option><option>Sekretaris Desa</option><option>Kaur Umum</option><option>Belum Didisposisi</option>
                    </select>
                  </div>
                </div>
              </div>

              <div className="p-5 lg:p-6 border-t border-border flex justify-end gap-3 shrink-0 bg-white rounded-b-2xl">
                <button onClick={closeModal} className="px-5 py-2.5 border border-hijau-light border-2 text-hijau rounded-xl font-bold text-sm hover:bg-hijau-pale transition cursor-pointer">Batal</button>
                <button onClick={() => { alert("Surat Disimpan!"); closeModal(); }} className="px-5 py-2.5 bg-hijau text-white rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-hijau-tua shadow-md transition cursor-pointer hover:-translate-y-[1px]">
                  Simpan Surat
                </button>
              </div>
