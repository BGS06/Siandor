...
export default function PenggunaPage() {
  ...
  const [users, setUsers] = useState([
    { jabatan: "Kepala Desa", username: "kepala_desa", role: "Eksekutif", color: "bg-emas" },
    { jabatan: "Sekretaris Desa", username: "sekretaris", role: "Operasional", color: "bg-biru" },
    { jabatan: "Operator Administrasi", username: "admin", role: "Admin", color: "bg-hijau-muda" },
  ]);
        ...
        <div className="p-4 lg:p-5 border-b border-border flex items-center justify-between gap-4 flex-wrap bg-white">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              placeholder="Cari jabatan atau username..." 
            />
          </div>
        </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full ${user.color} flex items-center justify-center font-bold text-sm text-white shrink-0`}>
                        {user.jabatan.substring(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <div className="text-sm font-bold text-hitam">{user.jabatan}</div>
                      </div>
                    </div>
                  </td>
